...
//                        'action' => \yii\helpers\Url::to(['registro/register'])
                ...
                <?= $form->field($model->docente, 'primer_nombre')->textInput(['maxlength' => true]) ?>

                <?= $form->field($model->docente, 'primer_apellido')->textInput(['maxlength' => true]) ?>

                <?= $form->field($model->docente, 'cedula')->textInput(['maxlength' => true]) ?>

                <?= $form->field($model->docente, 'telefono')->textInput(['maxlength' => true]) ?>

                <?= $form->field($model->docente, 'titulodocentes')->widget(
                    \dosamigos\selectize\SelectizeDropDownList::class,
                    [

                        'items' => \yii\helpers\ArrayHelper::map(\app\models\TituloProfesional::find()->all(), 'id', 'nombre'),
                        'options' => [
